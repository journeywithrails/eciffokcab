...
pdf.text  "Customer: #{customer.firstname} #{customer.lastname}"
   ...
   pdf.text "Phone: #{ActionController::Base.helpers.number_to_phone(customer.phone)}"
...
# this floats off to side - /1.5 means more than .5 to the right
 pdf.float do
    pdf.bounding_box [pdf.bounds.width / 1.5, pdf.bounds.top], :width => 300 do
      ...
      #footer floating
      pdf.move_down(10)
      pdf.text "Subtotal: #{ActionController::Base.helpers.number_to_currency(@invoice.subtotal)}", :size => 16, :style => :bold
      pdf.text "Tax: #{ActionController::Base.helpers.number_to_currency(@invoice.tax)}", :size => 16, :style => :bold
      pdf.text "Invoice Total: #{ActionController::Base.helpers.number_to_currency(@invoice.total)}", :size => 16, :style => :bold
...
items = @invoice.line_items.map do |item|
  [
    ...
    ActionController::Base.helpers.number_to_currency(item.price * item.quantity),
  ]
end
...
# Signature line
pdf.text  "Signed: ______________________________________________  Date:___________"
pdf.text  "I Acknowledge above work was approved."
